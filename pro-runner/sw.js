...
self.addEventListener('activate', (event) => {
  event.waitUntil((async () => {
    ...
    await Promise.all(keys.filter((key) => key.startsWith('pro-runner-shell-') && key !== CACHE_NAME).map((key) => caches.delete(key)));
    await self.clients.claim();
    broadcast({ type: 'PRO_RUNNER_SW_READY', version: SHELL_VERSION });
  })());
});
...
async function serveVirtual(request, projectId, rawPath) {
  ...
  if (!project) return new Response('Pro Runner: project not found.', { status:404, headers:{'Content-Type':'text/plain; charset=utf-8'} });
...
async function shellResponse(request) {
  ...
  if (cached) {
    fetch(request).then((response) => { if (response?.ok && request.method === 'GET') cache.put(request, response.clone()); }).catch(() => {});
    return cached;
  }
  try {
