...
export function createHomeController({ home, content, grid, dock, done, load, save, onOpen, onEdit, onDelete }) {
  let editing = false; let drag = null; let marker = null; let ghost = null; let dockMarkers = []; let suppressUntil = 0;
  ...
  function setEditing(value) {
    ...
    for (const [index, node] of items().entries()) {
      ...
      node.style.setProperty('--jiggle-shift', `${widget ? 0.12 + seed * 0.04 : 0.48 + seed * 0.17}px`);
      node.style.setProperty('--jiggle-angle', `${widget ? 0.72 + seed * 0.09 : 1.9 + seed * 0.2}deg`);
      node.style.setProperty('--jiggle-duration', `${widget ? 235 + seed * 10 : 195 + seed * 14}ms`);
      node.style.setProperty('--jiggle-delay', `${-seed * 53}ms`);
  ...
  function clearVisual() { marker?.remove(); marker = null; ghost?.remove(); ghost = null; dockMarkers.forEach((slot) => slot.remove()); dockMarkers = []; home.classList.remove('home-dock-drop'); }
  ...
  function begin(event) { drag.started = true; clearTimeout(drag.longTimer); ghost = drag.node.cloneNode(true); ghost.className = 'pro-drag-ghost'; ghost.querySelectorAll('[id]').forEach((node) => node.removeAttribute('id')); ghost.querySelector('.home-app-label')?.remove(); ghost.style.width = `${drag.node.querySelector('.home-app-icon')?.getBoundingClientRect().width || 64}px`; ghost.style.height = `${drag.node.querySelector('.home-app-icon')?.getBoundingClientRect().height || 64}px`; const sourceStyle = getComputedStyle(drag.node); const homeStyle = getComputedStyle(home); for (const property of ['--jiggle-shift','--jiggle-angle','--jiggle-duration','--jiggle-delay']) ghost.style.setProperty(property, sourceStyle.getPropertyValue(property)); ghost.style.setProperty('--spring-icon-radius', homeStyle.getPropertyValue('--spring-icon-radius') || '25.9%'); document.body.append(ghost); drag.node.classList.add('portal-drag-source'); try { home.setPointerCapture(event.pointerId); } catch {} }
  function move(event) { if (!drag || drag.pointer !== event.pointerId) return; const distance = Math.hypot(event.clientX - drag.x, event.clientY - drag.y); if (!drag.started && !editing) { if (distance > 14) clearTimeout(drag.longTimer); return; } if (!drag.started && distance >= 6) begin(event); if (!drag.started) return; event.preventDefault(); const left = event.clientX - drag.offsetX; const top = event.clientY - drag.offsetY; ghost.style.transform = `translate3d(${left}px,${top}px,0) scale(1.05)`; drag.target = target(event, drag.node); if (drag.target.zone === 'grid') { dockMarkers.forEach((slot) => slot.remove()); dockMarkers = []; markerAt(drag.target.position, drag.target.size, true); } else { marker?.remove(); marker = null; showDockMarkers(drag.target.index); home.classList.add('home-dock-drop'); } }
  ...
  for (const type of ['contextmenu', 'dragstart', 'selectstart']) home.addEventListener(type, (event) => { if (event.target.closest('[data-home-item="true"]')) { event.preventDefault(); event.stopPropagation(); } }, true);
