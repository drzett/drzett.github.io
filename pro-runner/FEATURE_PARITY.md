...
| Website viewer | Embedded website launch, reload, navigation controls and an external-browser fallback where embedding is blocked. | **Partial**: the viewer opens with controls closed and cannot bypass third-party CSP/X-Frame-Options. A best-effort CORS-readable header scan preselects browser launch only for definitively blocked sites; otherwise the result is shown as unverified and remains user-overridable. | `core/website-scan.js`, `main.js` |
