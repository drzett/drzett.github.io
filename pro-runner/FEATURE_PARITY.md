...
| Website viewer | Embedded website launch, reload, navigation controls and an external-browser fallback where embedding is blocked. | **Partial**: the viewer cannot bypass third-party CSP/X-Frame-Options, but now always presents an explicit “Open in browser” fallback for website shortcuts instead of leaving a silent black frame. | website viewer module |
