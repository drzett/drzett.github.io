...
test('activation cleans legacy shell caches and reports consistent runtime metadata', async ({ page }) => {
  ...
  expect(state.version).toMatchObject({ version: '2.0.2', build: '2026-09-18.5', releaseId: '2.0.2-2026-09-18.5' });
  expect(state.databaseVersion).toBe(2);
  expect(state.caches).toContain('pro-runner-app-shell-2.0.2-2026-09-18.5');
...
test('upgrades from the previously deployed worker without losing browser data', async ({ page, request }) => {
  ...
  try {
    ...
    await expect(page.locator('#updateCheckStatus')).toHaveText('Version 2.0.2 is ready to install.');
    const loaded = page.waitForEvent('load');
    await page.locator('#installUpdateButton').click();
    await loaded;

    await expect(page.locator('#installedVersionValue')).toHaveText('2.0.2');
    await expect(page.locator('.project-card', { hasText: 'upgrade-data' })).toBeVisible();
    const cachesAfter = await page.evaluate(() => caches.keys());
    expect(cachesAfter).toContain('pro-runner-app-shell-2.0.2-2026-09-18.5');
