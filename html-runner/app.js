  ...
  return new Promise((resolve) => {
    ...
    for (const path of [...htmlPaths].sort((a, b) => a.localeCompare(b))) {
      ...
      button.addEventListener('click', () => {
        finish(path);
        els.entryDialog.close('selected');
      });
...
async function importFolder(fileList) {
  ...
  setStatus('busy', 'Importing project folder…', `Storing ${files.length.toLocaleString()} files locally.`);
  await askForPersistentStorage();

  let entries = files.map((file) => ({
    file,
    relativePath: sanitizePath(file.webkitRelativePath || file.name),
  }));
  entries = stripCommonTopDirectory(entries);

  const records = entries.map(({ file, relativePath }) => ({
    path: sanitizePath(relativePath),
    blob: file,
    type: guessMime(relativePath, file.type),
    size: file.size,
    lastModified: file.lastModified || Date.now(),
  }));

  const htmlPaths = records
    .map((record) => record.path)
    .filter((path) => /\.(html?|xhtml)$/i.test(path));
  const entryPath = await chooseEntry(htmlPaths);

  if (!entryPath) {
    setStatus('ready', 'Ready', 'Import cancelled.');
    return;
  }

  const totalBytes = records.reduce((sum, record) => sum + record.size, 0);
  const topFolder = files[0]?.webkitRelativePath?.split('/')[0] || 'Web project';
  const project = {
    kind: 'folder',
    name: topFolder,
    entryPath,
...
async function openProject() {
  if (!currentProject || !swReady) return;
...
function openDirectMode() {
  ...
  location.href = virtualURL(currentProject.entryPath);
}
...
function toggleViewerMenu() {
  const open = els.viewerMenu.classList.toggle('hidden') === false;
...
async function forgetProject() {
  if (!currentProject) return;
  const confirmed = confirm('Remove the locally imported copy? The original files on your device will not be changed.');
  if (!confirmed) return;
...
function wireEvents() {
  ...
  els.filePicker.addEventListener('change', async () => {
    ...
    try { await importSingleFile(file); } catch (error) { handleError(error); }
  });

  els.folderPicker.addEventListener('change', async () => {
    const files = els.folderPicker.files;
    try { await importFolder(files); } catch (error) { handleError(error); }
    els.folderPicker.value = '';
  });

  els.resumeButton.addEventListener('click', openProject);
  els.menuButton.addEventListener('click', toggleViewerMenu);
  els.reloadButton.addEventListener('click', () => {
    if (!currentProject) return;
    els.viewer.src = `${virtualURL(currentProject.entryPath)}?reload=${Date.now()}`;
    closeViewerMenu();
  });
  ...
  els.storageButton.addEventListener('click', showStorageInfo);
  els.forgetButton.addEventListener('click', forgetProject);
  ...
  els.spaFallbackToggle.addEventListener('change', async () => {
  ...
  document.addEventListener('pointerdown', (event) => {
    if (!els.viewerMenu.classList.contains('hidden') && !els.viewerMenu.contains(event.target) && event.target !== els.menuButton) {
      closeViewerMenu();
    }
  });
  ...
  window.addEventListener('drop', async (event) => {
    ...
    if (event.dataTransfer?.files?.length === 1) {
      try { await importSingleFile(event.dataTransfer.files[0]); } catch (error) { handleError(error); }
    }
  });
}
...
async function init() {
  ...
  try {
    ...
    if (!folderImportSupported) {
      els.openFolderButton.title = 'Folder import is not supported by this browser.';
    }
    ...
    if (currentProject && els.autoOpenToggle.checked && isStandalone()) {
      requestAnimationFrame(() => setTimeout(openProject, 80));
    }
  } catch (error) {
    handleError(error);
  }
